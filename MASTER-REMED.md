...
| **Q12** | P-20 (terminaison des états I/O) et P-21 (liveness post-vente) référencent des constantes business (`max_attempts`, `retractSla(canal)`) non définies : ni dans le modèle de référence, ni comme valeurs concrètes dans ADAPTER-CONTRACT/SYNC-FSM. Les figer dans le modèle sans revue reviendrait à inventer une config opérationnelle à chaud — décision explicitement refusée (2026-07-15). Laissées en attente ; à trancher en revue à froid avant implémentation de P-20/P-21 | Implémentation P-20/P-21 (modèle de référence) |
