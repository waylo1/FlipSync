...
describe('ShopifyConnector — Admin API GraphQL', () => {
  ...
  it('userErrors productCreate → REMOTE_REJECTED avec detail', async () => {
    ...
    if (!outcome.ok) expect(outcome.detail).toContain('Title is invalid')
  })
  ...
  it('pivot sans photo (gate Zod media) → INVALID_PAYLOAD, zéro appel réseau', async () => {
    ...
    expect(await connector.publish({ ...LISTING, photos: [] })).toMatchObject({
      ok: false,
  ...
  it('withdraw → productUpdate ARCHIVED, succès normalisé', async () => {
    const { fn, calls } = fetchQueue([
      { status: 200, body: { data: { productUpdate: { product: { id: GID }, userErrors: [] } } } },
    ])
    const connector = new ShopifyConnector({ fetchFn: fn, env: ENV })

    expect(await connector.withdraw(GID)).toEqual({ ok: true, externalId: GID, url: null })
  ...
  it('update / checkStatus → CONNECTOR_UNAVAILABLE (v1 : publish + withdraw)', async () => {
    ...
    expect(await connector.update(GID, LISTING)).toMatchObject({
      ok: false,
      code: SyncErrorCode.CONNECTOR_UNAVAILABLE,
    })
    expect(await connector.checkStatus(GID)).toMatchObject({
      ok: false,
      code: SyncErrorCode.CONNECTOR_UNAVAILABLE,
    })
  })
